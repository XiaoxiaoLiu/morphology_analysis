...
sb.barplot(data=df_data,x='image_id', y='Tubularity', order = df_data.image_id,    ax=ax2)
#plt.xticks(range(len(df_data)), df_data['meta'], rotation='vertical')

ax1.set_xlabel('')
ax2.set_xlabel('')
sb.barplot(data=df_data,x='image_id', y='Dynamic_Range', order = df_data.image_id,    ax=ax3)
plt.xticks(range(len(df_data)), df_data['meta'], rotation='vertical')
