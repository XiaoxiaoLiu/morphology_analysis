...
import uuid
def execute_command(arguments, GEM_QSUB = 0, qsub_script_folder = "."):
    if GEN_QSUB :
        cmd = QMasterV3D + arguments
        #print cmd
        job_id = uuid.uuid1()
        script_fn = qsub_script_dir +'/'+job_id+'.qsub'
        gen_qsub_script(cmd, job_id, script_fn)
    else:
        cmd = V3D + arguments
        print cmd
        command = Command(cmd)
...
#===================================================================
def image_profiling(input_image_file,output_csv_file, dialation_ratio=3, flip=0, invert = 1):
    output_dir = os.path.dirname(output_csv_file)
    logfile = output_csv_file +'.log'
    if not os.path.exists(output_dir):
        os.system("mkdir -p  " + output_dir)
        print "create output dir: ", output_dir
o
...
def pre_processing(inputswc_path, outputswc_path, GEN_QSUB = 0, qsub_script_dir = "."):  # prune align x-axis and resample
    output_dir = os.path.dirname(outputswc_path)
    if not os.path.exists(output_dir):
        os.system("mkdir -p  " + output_dir)
        print "create output dir: ", output_dir
...
def batch_compute(input_linker_fn, feature_fn, GEN_QSUB = 0, qsub_script_dir = "."):  # prune align x-axis and resample:
    arguments = " -x blastneuron  -f batch_compute -p \" #i " + input_linker_fn + " #o " + feature_fn + "\" "
    execute_command(arguments,GEN_QSUB,qsub_script_dir)
    return
