source("/data/informatics/changkyul/Ephys/Ephys_Unix/analysis.function.Ephys.r")
source("/data/informatics/changkyul/Ephys/Ephys_Unix/vegan/R/screeplot.prcomp.R")
source("/data/informatics/changkyul/Ephys/Ephys_Unix/vegan/R/bstick.prcomp.R")
source("/data/informatics/changkyul/Ephys/Ephys_Unix/vegan/R/bstick.default.R")
...
BuildBinaryClusTree <- function(Nodei, soi.leg, Nshuffled=1000, flagDEC="LDA", flagGRP="SEL", flagPthr=0.05, flagSparse=FALSE, flagPlot=TRUE, flagDIP=FALSE ) {
if(0) {
Nodei <- Nodei$Right
Nshuffled=1000
flagDEC="LDA"
...
flagDebug=TRUE 
    ...
    if (!is.matrix(Nodei$Xsoi) || nrow(Nodei$Xsoi) <= TN.Sample.thr) {
        ...
        Nodei <- clustering2decR(Nodei, Nodei$Xsoi, Nodei$GT_cre, Nodei$ZSel35, Nodei$strin, Nodei$pch, soi.leg$pch, Nodei$crecolor, Nodei$hybrid.crecolorL, 
                            soi.leg$crecolor, soi.leg$crecolorL, soi.leg$str, soi.leg$strL, Nodei$NodeStr, Nshuffled, flagDEC, flagGRP, flagPthr, flagSparse, flagPlot, flagDIP) 
...
#        Nodei$Enough <- est_partition_boot(Nodei, seq(10,50,5), 100, 4, Node0.strin) 
        ...
        if (Nodei$nPC > 0) {
            ...
            Nodei$Left <- BuildBinaryClusTree ( Nodei$Left, soi.leg, Nshuffled, flagDEC, flagGRP, flagPthr, flagSparse, flagPlot, flagDIP )
    
            print("Right") 
            NodeRStr <- paste(Nodei$Str, "R", sep="")
            Nodei$Right <- SetUpChild( Nodei, Nodei$idxR, "R" )
            Nodei$Right <- BuildBinaryClusTree ( Nodei$Right, soi.leg, Nshuffled, flagDEC, flagGRP, flagPthr, flagSparse, flagPlot, flagDIP )
...
binary_partition_incPC <- function (Pin, nPC, meth, flagSparse=FALSE) {
...
if (debug==1) {
    ...
    if (NN1 > 0) {
        ...
        partLR <- binary_partition_incPC (Pin, NN1, "ward", flagSparse) 
        if (0) {
        grp12   <- matrix(0, nrow(Zin), NN1)
        xcindex <- Pxcindex <- rep(1, NN1)
        pnorm   <- rep(1,NN1)
            for (Npc in 1:NN1) {
                 print(Npc)
                 newdistmat<-matrix(0,nrow=nrow(Pin$x),ncol=nrow(Pin$x));
                 ...
                 hhh=hclust(as.dist(newdistmat),method="ward")
                 hhh_2grp <- cutree(hhh,k=2)
        
                 ntry <- 2*nrow(Zin)
                 if (Npc==1) {
                     sigval <- sigclust(cbind(coordmat,coordmat), ntry, labflag=1, label=as.vector(hhh_2grp), icovest=2) 
                 } else {
                     pcamat <- prcomp(coordmat)$x[,1] 
                     #sigval <- sigclust(coordmat, ntry, labflag=1, label=as.vector(hhh_2grp), icovest=2) 
                     sigval <- sigclust(cbind(pcamat,pcamat), ntry, labflag=1, label=as.vector(hhh_2grp), icovest=2) 
                 }
             
                 print(c(Npc,sigval@xcindex))
                 grp12[,Npc] <- hhh_2grp
                 if (0) {
                     if (sum(hhh_2grp==1) < sum(hhh_2grp==2)) {
                         grp12[,Npc] <- 3 - hhh_2grp
                     } else {
                         grp12[,Npc] <- hhh_2grp
                     }
                 }
                 xcindex[Npc] <- sigval@xcindex
                 pnorm[Npc] <- -log10(sigval@pvalnorm)
                  
                 if( sigval@xcindex < xcindex_min ) {
                     xcindex_min <- sigval@xcindex
                     min_Npc <- Npc
                     min_2grp <- grp12[,Npc]
                 }
            }
            ...
            if (flagPlot) { 
                barplot(summary(Pin)$importance[2,])
                par(mfrow=c(2,1))
                plot(1:Npc, pnorm, main="p-value", ylab="-log10(Pval)", xlab="number of PC's", pch=15)
                grid()
                plot(1:Npc, xcindex, main="C.Index = RSS.within/RSS.total", ylab="C.Index", xlab="number of PC's")
                grid()
            }
            #legend("bottomright", legend=c("weightedPC", "PC"),col=c("black","red"))
            print(paste("Npc=",min_Npc))
        }
...
clustering2decR <- function (ORIG, orig.XSel,orig.GT_cre, orig.ZSel35, orig.strin, mypch, leg.pch, crecolor, crecolor2, leg.crecolor, leg.crecolor2, leg.str, leg.str2, nodestr, nsim, flagDEC="LDA", flagGRP="SEL", flagPthr=0.05, flagSparse=FALSE, flagPlot=TRUE, flagDIP=FALSE) { 
