...
hhclust <- function (d, method = "ward", members = NULL) 
{
    ...
    hcl <- .Fortran("hc", n = as.integer(n), len = as.integer(len), 
        method = as.integer(method), ia = integer(n), ib = integer(n), 
        crit = double(n), membr = as.double(members), nn = integer(n), 
        disnn = double(n), flag = logical(n), diss = as.double(d), 
        PACKAGE = "flashClust")
    hcass <- .Fortran("hcass2", n = as.integer(n), ia = as.integer(hcl$ia), 
        ib = as.integer(hcl$ib), order = integer(n), iia = integer(n), 
        iib = integer(n), PACKAGE = "flashClust")
...
plot_these <- function( inresubcv.mean, inresubcv.feat, ZZZ, idx0, idx1, legstr, titlestr, crecolor2, leg.str, leg.crecolorIE, nselfeat=4, XSel ) {
    ...
    par(mfrow=c(1,1))
    h = heatmap.3(t(ZZZ[, selected.featnameshort[1:nselfeat]]), Rowv=FALSE, hclustfun=hhclust, trace="none", ColSideColors=crecolor2, #RowSideColors=crecolorIE, dendrogram="col", keysize=1,
...
heatmap.3 <- function (x, Rowv = TRUE, Colv = if (symm) "Rowv" else TRUE, 
    distfun = dist, hclustfun = hhclust, dendrogram = c("both", 
        "row", "column", "none"), symm = FALSE, scale = c("none", 
        "row", "column"), na.rm = TRUE, revC = identical(Colv, 
        "Rowv"), add.expr, breaks, symbreaks = min(x < 0, na.rm = TRUE) || 
        scale != "none", col = "heat.colors", colsep, rowsep, 
