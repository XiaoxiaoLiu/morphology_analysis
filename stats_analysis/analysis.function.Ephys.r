...
get_cluster_specific_features <- function (OUTDIR0, XXX, Id.id, pthr=0.01) { 
  ...
  for (lbl in c(unique.label)) {
    ...
    if (length(myt.adjp.idx) >1) {
      ...
      browser()
      print(lbl)
      print(names(t.pval[t.pval.order])[t.pval[t.pval.order] < pthr] )
      ######### ????? t.featname could be empty?
      print(t.featname)
      print(t.pval.order)
      print(t.pval)
...
gather_AssignedID_plotHeatmap_tree <- function(CLSDir, infolder, pthr, Node0, leg, filekey) {
  
  print(names(leg))
  print(names(Node0))
  print('###########################################################################')
  print('######## 1: PUT TOGETHER RESULTS  ###########') 
  print('###########################################################################')
  ...
  write.csv(data.frame(samplename=outSampleName, clusterID=outID), file=paste(CLSfolder, '/', infolder, '.', length(ttt), '.csv', sep=""), row.names=FALSE)
  write.csv(ttt, file=paste(CLSfolder, '/', infolder, '.', length(ttt), '.table.csv', sep=""))
  ...
  pdf(paste(CLSfolder, '/FinalNode.Heatmap.ClusterSpecificFeature.', filekey, '.pdf', sep=""), height=10, width=20)
  ...
  heatmap.3(t(X35[idx,gdx]), hclustfun=hclust, trace="none", Colv=FALSE, ColSideColors=color.2row, 
            Rowv=TRUE, main=paste("Iterative PCA based Clustering \n with Cluster Specific Features, adjp<", pthr), keysize=0.8, margins=c(10,13), cexCol=0.5,cexRow=1, col=rbg)
...
kyle_fem <- function ( featall, grpLR, Ks=c(2,3,4), modelin="all", methodin="svd", initin="kmeans", startin=10, flag.disp=FALSE, flag.debug=FALSE, ...) {
  ...
  if (is.singular.matrix(S)) {
    ...
    log_lik <- -(((Nfeat)*log(2*pi)) + log(norm(S))) * (Nsample / 2)
    for (i in c(1:Nsample)) {
      dx <- X[,i] - MU 
      log_lik <- log_lik - ((t(dx) %*% Sinv) %*% dx)
    }
    
    aic <- (-2*log_lik) + (2*Nparam)
    icl <- log_lik - log(Nsample)#stopping critira
...
clusterPCA <- function (Pin, NN1, crecolorL, leg.crecolorL, leg.strL, PDFFN, flagPlot=TRUE) {
  ...
  hpca.d <- hclust(dist.Pin, method="ward")
  
  cor.Pin <- as.dist((1 + cor(t(Pin$x[, 1:NN1])))/2) 
  hpca.c <- hclust(cor.Pin, method="ward")
